...
function calcDarkFlow(){
	...
	if (tmp.ach[75].has) tmp.dc.flow = tmp.dc.flow.mul(1.1);
	if (tmp.ach[83].has) tmp.dc.flow = tmp.dc.flow.mul(1.2);
	if (tmp.ach[131].has) tmp.dc.flow = tmp.dc.flow.mul(1.5);
	if (player.tr.upgrades.includes(11) && !HCCBA("noTRU")) tmp.dc.flow = tmp.dc.flow.mul(tr11Eff()["dcf"]);
	if (player.tr.upgrades.includes(12) && !HCCBA("noTRU")) tmp.dc.flow = tmp.dc.flow.mul(tr12Eff());
	if (tmp.inf) {
		if (tmp.inf.upgs.has("4;1")) tmp.dc.flow = tmp.dc.flow.mul(2);
		if (tmp.inf.upgs.has("5;1")) tmp.dc.flow = tmp.dc.flow.mul(2);
		if (tmp.inf.upgs.has("4;6")) tmp.dc.flow = tmp.dc.flow.mul(2);
		if (tmp.inf.upgs.has("5;5")) tmp.dc.flow = tmp.dc.flow.mul(INF_UPGS.effects["5;5"]());
		tmp.dc.flow = tmp.dc.flow.mul(tmp.inf.asc.perkEff(1));
		if (tmp.inf.upgs.has("7;2")) tmp.dc.flow = tmp.dc.flow.mul(INF_UPGS.effects["7;2"]()["flow"]);
		if (tmp.inf.upgs.has("7;6")) tmp.dc.flow = tmp.dc.flow.mul(INF_UPGS.effects["7;6"]());
		if (tmp.inf.upgs.has("7;8")) tmp.dc.flow = tmp.dc.flow.mul(INF_UPGS.effects["7;8"]());
		if (tmp.inf.upgs.has("10;5")) tmp.dc.flow = tmp.dc.flow.mul(INF_UPGS.effects["10;5"]());
		if (tmp.inf.upgs.has("10;10")) tmp.dc.flow = tmp.dc.flow.mul(INF_UPGS.effects["10;10"]());
	}
	if (tmp.elm) {
		if (player.elementary.times.gt(0)) tmp.dc.flow = tmp.dc.flow.mul(tmp.elm.bos.z2.max(1));
		if (player.elementary.sky.unl) tmp.dc.flow = tmp.dc.flow.mul(tmp.elm.sky.pionEff[5]);
	}
