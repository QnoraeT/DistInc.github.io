function updateCoalGain(){
	let thisName = "Coal Gain"
	clearMultiList(thisName)
	showMultiList(thisName, player.rf.gte(1))
	let eff
	tmp.fn.gain = new Decimal(player.rf.gt(0)?1:0)
	eff = ExpantaNum.pow(2, player.rf.min(inFC(5)?1:(1/0))).sub(1)
	tmp.fn.gain = tmp.fn.gain.mul(eff)
	setMultiList(thisName, "Rocket Fuel Effect", `2^${showNum(player.rf.min(inFC(5)?1:(1/0)))} - 1 = ${showNum(eff)}x`, `${showNum(tmp.fn.gain)}`)
	if (player.furnace.upgrades[0].gte(1)) {
		eff = ExpantaNum.pow(tmp.fn1base, player.furnace.upgrades[0].mul(tmp.fn.upgPow))
		tmp.fn.gain = tmp.fn.gain.mul(eff);
		setMultiList(thisName, "Furnace Upgrade 1 Effect", `${showNum(tmp.fn1base)}^(${showNum(player.furnace.upgrades[0])} * ${showNum(tmp.fn.upgPow)}) = ${showNum(eff)}x`, `${showNum(tmp.fn.gain)}`)
	}
	if (modeActive("extreme+hikers_dream")){
		if (player.achievements.includes(16) && tmp.hd.totalMotive) {
			...
			tmp.fn.gain = tmp.fn.gain.mul(eff)
			setMultiList(thisName, "Achievement 16 Reward", `${showNum(eff)}x`, `${showNum(tmp.fn.gain)}`)
		}
		if (player.achievements.includes(26) && tmp.hd.enerUpgs[1]) {
			let eff = tmp.hd.enerUpgs[1].add(10).log10().pow(3)
			if (tmp.hd.enerUpgs[1].gt(250)) eff = eff.mul(3)
			tmp.fn.gain = tmp.fn.gain.mul(eff)
			setMultiList(thisName, "Achievement 26 Reward", `${showNum(eff)}x`, `${showNum(tmp.fn.gain)}`)
		}
	}
	if (player.tr.upgrades.includes(16) && !HCCBA("noTRU") && modeActive("extreme")) {
		eff = (inFC(3)||inFC(5))?1:TR_UPGS[16].current().co
		tmp.fn.gain = tmp.fn.gain.mul(eff);
		setMultiList(thisName, "Time Reversal Upgrade 16", `${showNum(eff)}x`, `${showNum(tmp.fn.gain)}`)
	}
	if (player.tr.upgrades.includes(33) && !HCCBA("noTRU") && (tmp.rockets?tmp.rockets.clPow:false)) {
		eff = new ExpantaNum(tmp.rockets.clPow.max(1)||1).min(inFC(5)?1:(1/0))
		tmp.fn.gain = tmp.fn.gain.mul(eff)
		setMultiList(thisName, "Time Reversal Upgrade 33", `${showNum(eff)}x`, `${showNum(tmp.fn.gain)}`)
	}
	if (inFC(2)) {
		tmp.fn.gain = tmp.fn.gain.pow(0.075)
		setMultiList(thisName, "Furnace Challenge 2 Nerf", `^${showNum(0.075)}`, `${showNum(tmp.fn.gain)}`)
	}
	if (extremeStadiumActive("flamis", 5)) {
		tmp.fn.gain = tmp.fn.gain.pow(0.2)
		setMultiList(thisName, "Flamis Stadium Level 5 Nerf", `^${showNum(0.2)}`, `${showNum(tmp.fn.gain)}`)
	}
	if (tmp.ach[131].has) {
		tmp.fn.gain = tmp.fn.gain.mul(100)
		setMultiList(thisName, "Achievement 131 Reward", `${showNum(100)}x`, `${showNum(tmp.fn.gain)}`)
	}
}
...
function updateFurnaceUpgradeCosts() {
	...
	for (let n = 1; n <= 5; n++) {
			let scalFN
			let specialScale = [2, 3, 1.025, 5, 7]
			scalFN = player.furnace.upgrades[n - 1]
			scalFN = doAllScaling(scalFN, "fn", false, specialScale)
			scalFN = ExpantaNum.pow(tmp.fn.upgs[n].base.div(10), scalFN.root(tmp.fn.bfEffRecip.div(2))).mul(tmp.fn.upgs[n].base);
...
function updateBlueFlameEff() {
	...
	if (player.tr.upgrades.includes(17) && !HCCBA("noTRU") && modeActive("extreme"))
		adj = adj.mul(TR_UPGS[17].current());
	if (player.tr.upgrades.includes(26) && !HCCBA("noTRU") && modeActive("extreme"))
		adj = adj.mul(TR_UPGS[26].current());
	if (inFC(5)) adj = adj.mul(0.725)
	if (extremeStadiumActive("quantron", 2)) adj = adj.mul(0.95)
	tmp.fn.bfEff = ExpantaNum.div(1, player.furnace.blueFlame.add(tmp.fn.enh?tmp.fn.enh.eff:0).mul(adj).div(4).add(1));
	tmp.fn.bfEffRecip = player.furnace.blueFlame.add(tmp.fn.enh?tmp.fn.enh.eff:0).mul(adj).div(4).add(1)
	if (tmp.fn.bfEff.lt(0.0098)) tmp.fn.bfEff = tmp.fn.bfEff.cbrt().mul(0.0098 ** (2/3));
...
function updateFurnUpgEffs() {
	...
	if (FCComp(5)) tmp.fn4base = tmp.fn4base.add(ExpantaNum.mul(0.0001, player.furnace.upgrades[0].mul(tmp.fn.upgPow)))
	if (tmp.fn.enh) if (tmp.fn.enh.unl) {
		tmp.fn4base = tmp.fn4base.add(ExpantaNum.mul(tmp.fn.enh.upg3eff?tmp.fn.enh.upg3eff:0, player.furnace.enhancedUpgrades[2].add(tmp.fn.enh.upgs[3].extra)).mul(tmp.fn.enh.upgPow))
		if (player.elementary.entropy.upgrades.includes(25)) tmp.fn4base = tmp.fn4base.mul(ExpantaNum.pow(tmp.fn.enh.upg3eff?tmp.fn.enh.upg3eff:0, player.furnace.enhancedUpgrades[2].add(tmp.fn.enh.upgs[3].extra).mul(.2)))
	}
	...
	tmp.fn1base = new ExpantaNum(inFC(4)?1:(new ExpantaNum(FCComp(2)?28:3).add(ExpantaNum.mul(tmp.fn4base, player.furnace.upgrades[3].mul(tmp.fn.upgPow)))))
	if (player.elementary.entropy.upgrades.includes(25)) tmp.fn1base = tmp.fn1base.mul(ExpantaNum.pow(tmp.fn4base, ExpantaNum.mul(player.furnace.upgrades[3], .2)))
...
function updateCoalEff() {
	...
	if (tmp.ach[35].has) tmp.fn.eff = tmp.fn.eff.mul(2);
	if (player.tr.upgrades.includes(25) && !HCCBA("noTRU")) tmp.fn.eff = tmp.fn.eff.mul(2);
	if (player.tr.upgrades.includes(31) && !HCCBA("noTRU")) tmp.fn.eff = tmp.fn.eff.mul(1.8);
...
function updateBlueFlameCost() {
	...
	scalBFlame = ExpantaNum.pow(tmp.fn.bfLB, ExpantaNum.pow(tmp.fn.bfBase, scalBFlame).sub(1)).mul(1e6);
...
function updateEnhCoalGain() {
	tmp.fn.enh.gain = ExpantaNum.pow(1.1, tmp.fn.enh.unl?player.furnace.blueFlame:0).sub(1).mul(ExpantaNum.pow(tmp.fn.enh.upg1eff?tmp.fn.enh.upg1eff:1, player.furnace.enhancedUpgrades[0].add(tmp.fn.enh.upgs?tmp.fn.enh.upgs[1].extra:0).mul(tmp.fn.enh.upgPow)))
	if (tmp.ach[116].has) tmp.fn.enh.gain = tmp.fn.enh.gain.mul(ExpantaNum.pow(1.4, player.inf.pantheon.purge.power))
	if ((tmp.fn.enh.moltBr||new ExpantaNum(0)).gte(1)) tmp.fn.enh.gain = tmp.fn.enh.gain.mul(tmp.fn.enh.moltBrEff||1)
	if (tmp.ach[131].has) tmp.fn.enh.gain = tmp.fn.enh.gain.mul(100)
}

function updateEnhCoalEffs() {
	tmp.fn.enh.eff = tmp.fn.enh.unl?(player.furnace.enhancedCoal.add(1).log10().add(1).log10().add(1).log10().mul(5)):new ExpantaNum(0)
	tmp.fn.enh.eff2exp = ExpantaNum.mul(player.furnace.enhancedUpgrades[12], tmp.fn.enh.upg13eff?tmp.fn.enh.upg13eff:0).mul(tmp.fn.enh.upgPow).add(50)
...
function updateEnhFurnUpgCosts() {
	...
	for (let n = 1; n <= 13; n++) {
		...
		scalEFN = ExpantaNum.pow(tmp.fn.enh.upgs[n].base.div(10), scalEFN.div(tmp.fn.enh.upgs[n].costAdj)).mul(tmp.fn.enh.upgs[n].base);
		...
		tmp.fn.enh.upgs[n].extra = (n<=9?((player.furnace.enhancedUpgrades[n+2]||new ExpantaNum(0)).add(tmp.fn.enh.upgs[n+3]?(tmp.fn.enh.upgs[n+3].extra?tmp.fn.enh.upgs[n+3].extra:0):0).mul(tmp.fn.enh.upgPow)):new ExpantaNum(0)).mul((n>6)?2:1)
		if (n<13) tmp.fn.enh.upgs[n].extra = tmp.fn.enh.upgs[n].extra.add(player.furnace.enhancedUpgrades[12].mul(tmp.fn.enh.upgPow))
...
function updateEnhFurnUpgEffs() {
	...
	tmp.fn.enh.upg1eff = ExpantaNum.pow(ExpantaNum.add(3, ExpantaNum.mul(0.1, endMod)), player.furnace.coal.add(1).log10().add(1).log10().add(1).log10().add(1)).mul(ExpantaNum.pow(1.2, endMod))
	tmp.fn.enh.upg2eff = new ExpantaNum(0.9)
	tmp.fn.enh.upg3eff = new ExpantaNum(2.5)
	if (tmp.fn.pl) if (tmp.fn.pl.unl) {
		tmp.fn.enh.upg1eff = tmp.fn.enh.upg1eff.mul(tmp.fn.pl.boosts[7])
		tmp.fn.enh.upg3eff = tmp.fn.enh.upg3eff.mul(tmp.fn.pl.boosts[7])
	}
	tmp.fn.enh.upg13eff = ExpantaNum.add(30, endMod.div(3).floor().mul(10).min(10))
	...
	if (tmp.fn.pl) if (tmp.fn.pl.unl) tmp.fn.enh.upg13eff = tmp.fn.enh.upg13eff.mul(tmp.fn.pl.boosts[8])
}
...
function updateMoltenBricks() {
	...
	if (player.furnace.enhancedUpgrades[12].gte(6)) tmp.fn.enh.moltBr = tmp.fn.enh.moltBr.mul(player.furnace.enhancedUpgrades[12].mul(tmp.fn.enh.upgPow).sub(4).pow(0.6).max(1))
	if (player.elementary.times.gt(0) && tmp.fn.magma) tmp.fn.enh.moltBr = tmp.fn.enh.moltBr.mul(tmp.fn.magma.eff2)
	if (player.elementary.bosons.scalar.higgs.upgrades.includes("0;1;6")) tmp.fn.enh.moltBr = tmp.fn.enh.moltBr.mul(2)
