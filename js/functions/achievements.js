function checkAchievements(row) {
	switch (row) {
		case 1:
		...
		case 16:
			...
			if (player.distance.gte("4.4e108000026")) tmp.ach[167].grant();
		...
		case 20:
			break;
		default:
			throw new RangeError(`Achievements only have ${ACH_DATA.rows} rows! (Index ${row})`)
	}
}

function updateAchievements() {
	tmp.nopathogenupgs = getTotalPathogenUpgrades().eq(0)
	for (let r=1;r<=ACH_DATA.rows;r++) if (!Achievement.hasRow(r)) checkAchievements(r);
}
